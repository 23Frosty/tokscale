...
pub fn render(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let header_cells = if is_very_narrow {
        ...
        vec![
            "Month", "Range", "Turn", "Msgs", "Input", "Output", "Cache R", "Cache W", "Cache×",
            "Total", "Cost",
        ]
    } else {
        vec![
            "Month", "Range", "Msgs", "Input", "Output", "Cache R", "Cache W", "Cache×", "Total",
            "Cost",
        ]
    };
    ...
    let rows: Vec<Row> = monthly[start..end]
        ...
        .map(|(i, month)| {
            ...
            let is_current_month = today >= month.start_date && today <= month.end_date;
            let range = format!(
                "{}-{}",
                month.start_date.format("%m/%d"),
                month.end_date.format("%m/%d")
            );

            let cells: Vec<Cell> = if is_very_narrow {
                vec![
                    Cell::from(month.month.clone()).style(if is_current_month {
                        Style::default()
                            .fg(Color::Yellow)
                            .add_modifier(Modifier::BOLD)
                    } else {
                ...
                let mut cells = vec![Cell::from(month.month.clone()).style(if is_current_month {
                    Style::default()
                        .fg(Color::Yellow)
                        .add_modifier(Modifier::BOLD)
                } else {
                ...
                cells.extend([
                    Cell::from(month.message_count.to_string()),
                    Cell::from(format_tokens(month.tokens.input))
                        .style(Style::default().fg(Color::Rgb(100, 200, 100))),
                    Cell::from(format_tokens(month.tokens.output))
